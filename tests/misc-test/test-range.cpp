...
KE_TEST(OpenRange)
{
	...
	ASSERT_TRUE(ke::OpenRange<int>(1, 1).is_degenerate());
	ASSERT_TRUE(ke::OpenRange<float>(1.0f, 1.0f).is_degenerate());
	ASSERT_TRUE(ke::OpenRange<char>('a', 'a').is_degenerate());
	//ASSERT_TRUE(ke::OpenRange<std::string>("a", "a").is_degenerate());
	ASSERT_TRUE(ke::OpenRange<uint8_t>(-1, 1).is_degenerate());

	ASSERT_FALSE(rangei.is_degenerate());
	ASSERT_FALSE(rangef.is_degenerate());
	ASSERT_FALSE(rangec.is_degenerate());
	//ASSERT_FALSE(ranges.is_degenerate());
...
KE_TEST(ClosedRange)
{
	...
	//ASSERT_TRUE(ke::ClosedRange<std::string>("a", "a").is_degenerate());
	ASSERT_TRUE(ke::ClosedRange<uint8_t>(-1, 1).is_degenerate());
...
KE_TEST(HalfOpenRange)
{
	...
	ASSERT_TRUE(ke::HalfOpenRange<int>(1, 1).is_degenerate());
	ASSERT_TRUE(ke::HalfOpenRange<float>(1.0f, 1.0f).is_degenerate());
	ASSERT_TRUE(ke::HalfOpenRange<char>('a', 'a').is_degenerate());
	//ASSERT_TRUE(ke::HalfOpenRange<std::string>("a", "a").is_degenerate());
	ASSERT_TRUE(ke::HalfOpenRange<uint8_t>(-1, 1).is_degenerate());
...
KE_TEST(inRange)
{
	ASSERT_TRUE(ke::in_range(ke::OpenRange(-2, 2), 0));
	ASSERT_FALSE(ke::in_range(ke::OpenRange(-2, 2), 2));
	ASSERT_FALSE(ke::in_range(ke::OpenRange(-2, 2), -2));

	ASSERT_TRUE(ke::in_range(ke::ClosedRange(-2, 2), 0));
	ASSERT_TRUE(ke::in_range(ke::ClosedRange(-2, 2), 2));
	ASSERT_TRUE(ke::in_range(ke::ClosedRange(-2, 2), -2));

	ASSERT_TRUE(ke::in_range(ke::HalfOpenRange(-2, 2), 0));
	ASSERT_FALSE(ke::in_range(ke::HalfOpenRange(-2, 2), 2));
	ASSERT_TRUE(ke::in_range(ke::HalfOpenRange(-2, 2), -2));
}
...
{
	int a = 0;
	ASSERT_TRUE(ke::restrain_variable(a, 1, 10));
	ASSERT_EQUAL(a, 1);

	a = 0;
	ASSERT_TRUE(ke::restrain_variable(a, -10, -1));
	ASSERT_EQUAL(a, -1);

	a = 0;
	ASSERT_FALSE(ke::restrain_variable(a, 0, 0));
	ASSERT_EQUAL(a, 0);
	...
	// degenerate range
	a = 4;
	ASSERT_FALSE(ke::restrain_variable(a, 10, -10));
	ASSERT_EQUAL(a, 4);

	a = 0;
	ASSERT_TRUE(ke::restrain_variable(a, ke::ClosedRange(1, 10)));
	ASSERT_EQUAL(a, 1);

	a = 0;
	ASSERT_FALSE(ke::restrain_variable(a, ke::ClosedRange(0, 10)));
	ASSERT_EQUAL(a, 0);

	a = 0;
	ASSERT_FALSE(ke::restrain_variable(a, ke::ClosedRange(0, 0)));
	ASSERT_EQUAL(a, 0);
	...
	for (int i = 0; i < 100; i++)
	{
		...
		ke::restrain_variable(a, ke::ClosedRange(0, 100));
		ASSERT_TRUE(ke::in_range(ke::ClosedRange(0, 100), a));
	}

	for (char c = 'a'; c <= 'z'; c++)
	{
		char a = c;
		ke::restrain_variable(a, ke::ClosedRange('a', 'z'));
		ASSERT_EQUAL(a, c);
	}
	...
	for (char c = ' '; c <= '~'; c++)
	{
		char a = c;
		ke::restrain_variable(a, ke::ClosedRange('a', 'z'));
		ASSERT_TRUE(ke::in_range(ke::ClosedRange('a', 'z'), a));
	}

	for (float f = -100.0f; f <= 100.0f; f += 0.1f)
	{
		float a = f;
		ke::restrain_variable(a, ke::ClosedRange(-10.0f, 10.0f));
		ASSERT_TRUE(ke::in_range(ke::ClosedRange(-10.0f, 10.0f), a));
	}
}
